...
public class MoveBall : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _controllerManager.BlackHoleAdded += tmpBlackHoleGameObject =>
        {
            blackHole.Add(tmpBlackHoleGameObject);
        };
        _controllerManager.WhiteHoleAdded += tmpWhiteHoleGameObject =>
        {
            whiteHole.Add(tmpWhiteHoleGameObject);
        };
    ...
    void Update()
    {
        for (int i = 0; i < blackHole.Count; i++)
        {
            ...
            direction.Normalize();
            // 加速度与える
            GetComponent<Rigidbody>().AddForce(1 * direction, ForceMode.Acceleration);
        }
